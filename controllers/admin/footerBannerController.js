/*
 * Footer Banner Admin
 * Управляет нижними баннерами главной страницы и их медиа-файлами.
 */
const FooterBanner = require('../../models/FooterBanner');
const { processImage, deleteImage } = require('../../utils/imageHandler');

class FooterBannerController {
	async getSlides(req, res) {
		try {
			...
			const totalCount = await FooterBanner.countDocuments();


			const slides = await FooterBanner.find()
				.sort({ _id: -1 })
				.skip(skip)
				.limit(limit)
	...
	async getSlideById(req, res) {
		try {
			...
			const slide = await FooterBanner.findById(id).select('-__v').lean();
	...
	async createSlide(req, res) {
		try {
			...
			const { webp } = await processImage(file, 'FooterBanner');
			const imagePath = webp;


			const slide = new FooterBanner({
				name,
				image: imagePath,
				url,
			});
	...
	async updateSlide(req, res) {
		try {
			...
			const slide = await FooterBanner.findById(id);
			if (!slide) {
			...
			const updatedBanner = await FooterBanner.findByIdAndUpdate(
				id,
				updateData,
				{
					new: true,
	...
	async deleteSlide(req, res) {
		try {
			...
			const slide = await FooterBanner.findByIdAndDelete(id);
			if (!slide) {
				throw new Error('Слайд не найден');
			}
...
module.exports = new FooterBannerController();
